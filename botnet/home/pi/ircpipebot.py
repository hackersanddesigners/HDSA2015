...
class Bot (irc.bot.SingleServerIRCBot):
    ...
    def on_join(self, c, e):
        ...
        if nick != self.nickname:
            ...
            out = u"({0}) {1}: JOIN".format(tstr, nick)
            sys.stdout.write(out.encode("utf-8")+"\n")
            sys.stdout.flush()

    def on_part(self, c, e):
        # print ("PART", c, e, file=sys.stderr)
        nick = e.source.split("!", 1)[0]
        if nick != self.nickname:
            tstr = time.strftime("%H:%M:%S", time.localtime())
            out = u"({0}) {1}: PART".format(tstr, nick)
            sys.stdout.write(out.encode("utf-8")+"\n")
            sys.stdout.flush()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--channel', default='#botnet', help='channel to join (default: #botopera)')
